...
func TestPropose(t *testing.T) {
	...
	cfg := triage.RuleConfig{CurrentMajor: 5, KeepReactions: 20, Now: now}
	...
	tests := map[string]struct {
		...
		"legacy bug from label": {
			...
			wantAction: db.ActionClose, wantReason: triage.ReasonLegacyBug,
		},
		"legacy bug v3": {
			issue:      openIssue("bug"),
		...
		"recent claim blocks close": {
			...
			wantAction: db.ActionKeep, wantReason: triage.ReasonConfirmedRecent,
		},
		"open pr blocks close": {
			...
			wantAction: db.ActionKeep, wantReason: triage.ReasonHasOpenPR,
		},
		"high engagement blocks close": {
			...
			wantAction: db.ActionKeep, wantReason: triage.ReasonHighEngagement,
		},
		"recent version kept": {
			issue:      openIssue("bug", "v/4.x"),
			signals:    &db.Signals{Kind: "bug", VersionMajor: 4, VersionSource: "label", LastActivity: old},
			wantAction: db.ActionKeep, wantReason: triage.ReasonRecentVersion,
		},
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			got := triage.Propose(tc.issue, tc.signals, cfg)
			if got == nil {
	...
	t.Run("closed issue skipped", func(t *testing.T) {
		...
		if got := triage.Propose(i, &db.Signals{}, cfg); got != nil {
	...
	t.Run("multi version label lowers confidence", func(t *testing.T) {
		...
		got := triage.Propose(i, s, cfg)
		if got == nil || got.Reason != triage.ReasonLegacyBug {
