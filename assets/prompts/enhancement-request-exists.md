You are triaging OPEN enhancement requests for the terraform-provider-azurerm provider. Each numbered request below asked for something that now appears to exist in the provider: either the requested resource or data source is in the provider today (with the changelog bullet announcing its arrival and a documentation link — usually shipped after the request was filed, sometimes already available before it), or a property the request's prose names shipped for one of its resources. You are given the request's title, body, dates, and recent comments.
