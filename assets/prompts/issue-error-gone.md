...
- A comment credibly claims the problem still occurs on v{{CURRENT_MAJOR}} — the message may simply have been reworded while the behaviour survived. A claim on an older major does not protect by itself, but treat one from the last few months with caution for the same reason.
